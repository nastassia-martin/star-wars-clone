...
interface IProps {
	res: SW_VehiclesResponse
}

const Vehicles: React.FC<IProps> = ({ res }) => {
	if (res.total == 0) {
	...
	return (
		<>
			<p>
				Showing {res.from} to {res.to} of all {res.total} vehicles.
			</p>
			<Container className="p-4">
				<Row className="g-4">
					{res.data.map((d) => (
						<Card key={d.id} as={Link} to={`/vehicles/${d.id}`}>
							<Card.Body>
								<Card.Title>{d.name}</Card.Title>
								<ListGroup>
									<ListGroup.Item>Model: {d.model}</ListGroup.Item>
									<ListGroup.Item>
										Vehicles Class: {d.vehicle_class}
									</ListGroup.Item>
									<ListGroup.Item>
